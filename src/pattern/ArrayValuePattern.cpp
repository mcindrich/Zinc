...
namespace zinc {
namespace lang {
    bool ArrayValuePattern::matches(TokenIterator &beg, TokenIterator &end)
    {
        bool retval = true;
        TokenIterator beg_tmp = beg;

        // check for first token being word like 'simpleFunction'
        TokenPattern word_tp(TokenType::Word, ASTType::ArrayValue, 1);
        ...
        if (word_tp.matches(beg_tmp, end)) {
            ...
            if (beg_tmp->type == TokenType::LBracket) {
                ...
                while (beg_tmp != end && parenth_stack.size() > 0
                    ...
                    if (beg_tmp->type == TokenType::RBracket) {
                        parenth_stack.pop();
                    } else if (beg_tmp->type == TokenType::LBracket) {
                        parenth_stack.push(beg_tmp->type);
                    }
                    tmp_tokens.push_back(*beg_tmp);
                    beg_tmp++;
